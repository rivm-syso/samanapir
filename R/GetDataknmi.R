...
#'
#'LET OP: als er geen data beschikbaar was (of problemen met server) dan
#'is de return: NULL
#'
#' @export
#'
...
GetKNMIAPI2 <- function(stations, ymd_vanaf, ymd_tot){
  ...
  # Zet de lijstje stations om in 1 string mee te geven
  ...
  # Maak de request aan
  req <- request("https://www.daggegevens.knmi.nl/klimatologie/uurgegevens") |>
    req_body_form(
      stns = stns,
               start = ymdh_vanaf,
               end = ymdh_tot,
  ...
  if(substr(knmi_data_raw[[1]],1,1)!= "#"){
    logger::log_info('KNMI: Geen data opgehaald. ERROR')
    return(NULL)
  }

  logger::log_debug('KNMI: ruwe data opgehaald')

  # Splits uit per regel
  knmi_uur_raw <- stringr::str_split(knmi_data_raw, "\n")[[1]]

  # Bepaal aantal headers:
  aantal_headers <- length(grep("#",knmi_uur_raw))

  # Haal de kolomnamen op:
  # Dit gaat bijna goed: nog 2 schoonheidsfoutjes: '# STN' en 'TRUE'
  # (de T van temperatuur wordt vertaald naar TRUE)
  header_names <- as.character(read.csv(textConnection(knmi_uur_raw[aantal_headers]),
                                        header=F, strip.white=T, stringsAsFactors=F))
  header_names[1] <- 'STNS'
  header_names[grep("TRUE",header_names)] <- 'TEMP'

  # Zet de ruwe tekst om in een dataframe
  knmi_uur_df <- utils::read.csv(textConnection(knmi_uur_raw), header=F,
                                 skip=aantal_headers, na.strings="-",
                                 col.names=header_names)

  logger::log_debug('KNMI: data omgezet naar dataframe')
  ...
  knmi_uur_df['tijd'] <- as.POSIXct(as.character(knmi_uur_df$YYYYMMDD),
                                    tz='UTC', format='%Y%m%d')
  ...
  regeleinde_stns <- which(grepl("YYYYMMDD ", knmi_uur_raw))-1
  ...
  stn_info_df <- (utils::read.csv(textConnection(stn_info_ruw), header=F,
                                  sep = " ", strip.white=T,
                                  stringsAsFactors = F))

  logger::log_debug('KNMI: metadata omgezet naar dataframe')
  ...
  if(length(names(stn_info_df))==6){
    ...
    stn_info_df[6] <- NULL}else if(length(names(stn_info_df))==7){
  ...
  # Maak een list waarin de metingen en de coordinaten van de stations staan
  knmi_info_data <- list(info=stn_info_df, data=knmi_uur_df)

  logger::log_debug('KNMI: metadata en data gecombineerd')
